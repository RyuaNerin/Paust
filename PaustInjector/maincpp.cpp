...
int APIENTRY wWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPWSTR lpCmdLine, int cmdShow)
{
    ...
    if (hFFXIV == NULL)
    {
        MessageBoxW(NULL, L"No permission OR Final Fantasy XIV is not running", L"PaustInjector", 0);
        return 1;
    }
    ...
    if (isInjected(pid, L"PaustCore.dll", &pDllBaseAddress))
    {
        if (!uninjectDll(pid, pDllBaseAddress))
            MessageBoxW(NULL, L"un-injection failed.", L"PaustInjector", 0);
        else
            MessageBoxW(NULL, L"un-injected", L"PaustInjector", 0);
    ...
    else
    {
        ...
        path = path.parent_path().append(L"PaustCore.dll");

        if (!injectDll(pid, path.c_str()))
            MessageBoxW(NULL, L"injection failed.", L"PaustInjector", 0);
        else
            MessageBoxW(NULL, L"injected", L"PaustInjector", 0);
    }
